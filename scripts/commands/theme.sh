...
# ── Patch wezterm.lua colors in-place ──
# Replaces everything between THEME-COLORS-BEGIN and THEME-COLORS-END markers.
patch_wezterm_colors() {
  ...
  local tmp="$STATE_DIR/wezterm.lua.tmp"
...
EOF
  ...
  awk -v block="$block" '
    /THEME-COLORS-BEGIN/ { print; print block; skip=1; next }
    /THEME-COLORS-END/ { skip=0 }
    !skip { print }
  ' "$wezterm_config" > "$tmp"

  if [[ -s $tmp ]]; then
    cp "$tmp" "$wezterm_config"
  fi
}

# ── Patch ghostty/config colors in-place ──
# Replaces everything between THEME-COLORS-BEGIN and THEME-COLORS-END markers.
patch_ghostty_colors() {
  local bg=$(color background "#1a1b26")
  local fg=$(color foreground "#a9b1d6")
  local bright_fg=$(color bright_foreground "$fg")
  local selection_bg=$(color selection "$bg")
  ...
  local tmp="$STATE_DIR/ghostty.conf.tmp"
...
apply_theme() {
  ...
  # Wezterm and ghostty colors are patched in-place in their config files.
  # Neither supports safe runtime reload (wezterm SIGUSR1 crashes on this
  # build). They will pick up new theme colors on next launch/restart.
